    ...
    <AppLayout>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">{items.length} kelas terdaftar</p>
        </div>
      ...
      <div className="mt-5 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {items.map((c) => (
          <div key={c.id} className="rounded-2xl bg-white ring-1 ring-slate-200 p-5">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-lg font-bold text-slate-900">{c.name}</div>
                <div className="text-xs text-slate-500 mt-0.5">{c.grade_level || "—"}</div>
              </div>
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-primary-50 text-primary-700">{c.student_count} siswa</span>
            </div>
            <div className="mt-3 text-sm text-slate-600">
              Wali Kelas: <span className="font-medium text-slate-800">{c.teacher_name || "—"}</span>
            </div>
            <div className="mt-4 flex gap-2">
              <button onClick={() => openEdit(c)} className="btn-secondary-sm">Edit</button>
              <button onClick={() => handleDelete(c)} className="text-xs font-semibold px-3 py-1.5 rounded-lg text-rose-600 hover:bg-rose-50">Hapus</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={form.id ? "Edit Kelas" : "Tambah Kelas"}>
        <form onSubmit={handleSave} className="grid gap-3">
          <label className="block">
            <span className="block text-xs font-semibold text-slate-700 mb-1">Nama Kelas *</span>
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="contoh: TK A1" className="input-base" />
          </label>
          <label className="block">
            <span className="block text-xs font-semibold text-slate-700 mb-1">Jenjang / Tingkat</span>
          ...
          <label className="block">
            <span className="block text-xs font-semibold text-slate-700 mb-1">Wali Kelas</span>
            <select value={form.homeroomTeacherId ?? ""} onChange={(e) => setForm({ ...form, homeroomTeacherId: e.target.value ? Number(e.target.value) : null })} className="input-base">
