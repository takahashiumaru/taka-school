...
import { Academic, Assessments, Reports, Students, getUser, waLink, type AssessmentType, type GradeEntry, type PaudAspect, type PaudIndicator, type PaudObservation, type Report, type ReportCard, type Student, type Subject, type PaginationMeta } from "../lib/api"
  ...
  async function refresh() {
    ...
    try {
      const [r, s, g, o, c, m, a] = await Promise.all([
        Reports.list({ studentId: filterStudent || undefined, page, pageSize }), Students.list({ status: "aktif" }), Assessments.listGrades(filterStudent || undefined), Assessments.listObservations(filterStudent || undefined), Assessments.listCards(), Assessments.metadata(), Academic.metadata(),
      ])
      setItems(r.items); setPagination(r.pagination); setStudents(s.items); setGrades(g.items); setObservations(o.items); setCards(c.items); setTypes(m.assessmentTypes); setAspects(m.paudAspects); setIndicators(m.paudIndicators); setSubjects(a.subjects)
    } catch (e) { setError(e instanceof Error ? e.message : "Gagal") } finally { setLoading(false) }
  }
  useEffect(() => { refresh() }, [filterStudent, page, pageSize])
