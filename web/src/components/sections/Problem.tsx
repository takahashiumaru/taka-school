    ...
    <section className="section bg-slate-50">
      <div className="max-w-7xl mx-auto container-px">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((it) => (
            <div key={it.bad} className="rounded-2xl bg-white ring-1 ring-slate-200 p-5">
              <div className="flex items-start gap-3">
                ...
                <p className="text-slate-500 line-through">{it.bad}</p>
              </div>
              <div className="mt-3 flex items-start gap-3">
                <span className="mt-0.5 inline-flex h-6 w-6 flex-none items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </span>
                <p className="text-slate-800 font-medium">{it.good}</p>
              </div>
            </div>
          ))}
        </div>
